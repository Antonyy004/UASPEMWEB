...
if (isset($_GET['delete'])) {
    ...
    // Hapus balasan yang terkait dengan feedback ini
    $delete_replies = "DELETE FROM admin_replies WHERE feedback_id = $id";
    if ($conn->query($delete_replies) === TRUE) {
        echo "Balasan berhasil dihapus.<br>";
    } else {
        echo "Error deleting reply: " . $conn->error;
    }
    
    // Kemudian hapus feedback
...
// Cek jika form balasan admin disubmit
if (isset($_POST['submit_reply'])) {
    $feedback_id = $_POST['feedback_id'];
    $reply = $_POST['reply'];
    
    // Insert balasan baru
    $insert_reply = "INSERT INTO admin_replies (feedback_id, reply) VALUES ($feedback_id, '$reply')";
    $conn->query($insert_reply);
}

// Cek jika balasan ingin diperbarui
if (isset($_POST['edit_reply'])) {
    $reply_id = $_POST['reply_id'];
    $new_reply = $_POST['reply'];
    
    // Update balasan
    $update_reply = "UPDATE admin_replies SET reply = '$new_reply' WHERE id = $reply_id";
    ...
    <section class="content">
        ...
        <?php while ($row = $result->fetch_assoc()) { ?>
            <div class="feedback-item">
                ...
                <a href="admin.php?delete=<?php echo $row['id']; ?>">Delete Feedback</a>

                <!-- Menampilkan Balasan Admin di bawah Feedback -->
                <div class="admin-reply">
                    ...
                    if ($reply_result->num_rows > 0) {
                        // Jika ada balasan, tampilkan balasan tersebut
                    ...
                    ?>
                        ...
                        <form method="POST" action="admin.php">
                            ...
                            <textarea name="reply"><?php echo $reply['reply']; ?></textarea>
                            <input type="submit" name="edit_reply" value="Update Reply">
                        </form>
                    <?php
                    } else {
                    ?>
                        <!-- Jika tidak ada balasan, tampilkan form untuk balasan baru -->
                        <form method="POST" action="admin.php">
                            <input type="hidden" name="feedback_id" value="<?php echo $row['id']; ?>">
                            <textarea name="reply" placeholder="Your reply here..."></textarea>
                            <input type="submit" name="submit_reply" value="Submit Reply">
                        </form>
                    <?php
                    }
                    ?>
